...
internal static class StackCatalogJson
{
    internal static StackCatalogState Restore(StackCatalogDocument catalog)
    {
    ...
    internal static StackCatalogDocument CreateDocument(StackCatalogState state)
    {
        NoteOperations.Validate(state.Stacks);
        NoteRecovery.ValidateHistory(state.Stacks, state.DeletedNotes, state.NoteHistory);
    ...
    private static ItemDocument CreateItemDocument(DropItem item) => new()
    {
        ...
        Note = item.Note,
        AttachedNotes = [.. item.AttachedNotes]
    };
...
internal sealed record StackCatalogState(
    ...
    IReadOnlyList<DeletedNote> DeletedNotes,
    IReadOnlyList<NoteCaptureHistory> NoteHistory)
{
...
{
    public List<DeletedNote> DeletedNotes { get; set; } = [];

    public List<NoteHistoryDocument> NoteHistory { get; set; } = [];
...
internal sealed class NoteHistoryDocument
{
    public Guid NoteId { get; set; }
    public Guid SourceStackId { get; set; }
    public string SourceStackName { get; set; } = string.Empty;
    public ItemDocument SourceItem { get; set; } = new();

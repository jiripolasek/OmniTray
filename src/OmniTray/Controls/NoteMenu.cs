...
public sealed class NoteMenu : DependencyObject
{
    ...
    private static void OnStackMenuOpening(object? sender, object args)
    {
        ...
        if (GetShowAsSubmenu(menu))
        {
            ...
            if (notes is null)
            {
                notes = new MenuFlyoutSubItem { Text = "Notes", Icon = new FontIcon { Glyph = "\uE70B" }, Tag = new NoteMenu() };
        ...
        AddAction(menu, items, "New note", () =>
            App.Current.CreateNote(new NoteTarget(stack.Model.Id, NotePlacement.StackItem)), new FontIcon { Glyph = "\uE70B" });
        ...
        AddAction(menu, items, "Recently deleted notes", () => App.Current.ShowNotes(true), new SymbolIcon(Symbol.Undo));
    }
    ...
    internal static void PopulateItemMenu(MenuFlyout menu, DropStackViewModel stack, DropItem item)
    {
        ...
        AddAction(menu, menu.Items, "Attach a note to item", () =>
            App.Current.CreateNote(new NoteTarget(stack.Model.Id, NotePlacement.Item, item.Id)), new FontIcon { Glyph = "\uE70B" });
    ...
    private static void AddExisting(MenuFlyout menu, IList<MenuFlyoutItemBase> items, IReadOnlyList<StickyNote> notes)
    {
        ...
        {
            AddAction(menu, items, note.DisplayName, () => App.Current.ShowNote(note.Id), new FontIcon { Glyph = "\uE70B" });
        }
    }

    private static void AddAction(MenuFlyout menu, IList<MenuFlyoutItemBase> items, string text, Action action, IconElement icon)
    {

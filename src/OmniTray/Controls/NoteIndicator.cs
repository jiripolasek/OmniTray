...
public sealed partial class NoteIndicator : Button
{
    ...
    private void Refresh()
    {
        ...
        AutomationProperties.SetName(this,
            $"{notes.Count} {(this.Item is null ? "notes in stack" : "attached notes")}. Open notes.");
        ToolTipService.SetToolTip(this, string.Join("\n", notes.Take(3).Select(note =>
            note.Text.Length > 140 ? note.Text[..140] + "…" :
            string.IsNullOrWhiteSpace(note.Text) ? "New note" : note.Text)));
    }

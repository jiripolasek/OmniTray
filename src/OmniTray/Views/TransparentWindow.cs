...
/// <remarks>
///     <para>
///         The constructor applies all of the boilerplate that PowerToys overlays
///         currently hand-roll:
///     </para>
///     <list type="bullet">
...
///         <item>
///             Apply a <see cref="TransparentTintBackdrop" /> so the HWND is fully
///             see-through and the visible chrome can be drawn by the content.
///         </item>
///     </list>
///     <para>
///         This window is intentionally animation-agnostic: it does not own any
...
///         or hides immediately.
///     </para>
///     <para>
///         <b>Multiple surfaces.</b> More than one <see cref="TransientSurface" />
...
///         responsibility (this window owns no layout).
///     </para>
/// </remarks>
public partial class TransparentWindow : WindowEx
{
    ...
    protected TransparentWindow(bool useRoundedCorners)
    {
        this.AppWindow.Hide();
        ...
        this.ApplyTransparentChrome(useRoundedCorners);
    ...
    /// </summary>
    protected void ApplyTransparentChrome() => this.ApplyTransparentChrome(false);

    /// <summary>
    ///     Applies (or re-applies) the baseline transparent chrome, optionally retaining
    ///     rounded DWM corners for a compact content-sized surface.
    /// </summary>
    /// <param name="useRoundedCorners">
    ///     <see langword="true" /> for rounded compact surfaces; otherwise no DWM rounding.
    /// </param>
    protected void ApplyTransparentChrome(bool useRoundedCorners)
    {
        if (this._hwnd == 0)
        {
            return;
        ...
        HwndExtensions.ToggleWindowStyle(this._hwnd, false, WindowStyle.TiledWindow);
        HwndExtensions.ToggleWindowStyle(
            this._hwnd,
            true,
            WindowStyle.Popup | WindowStyle.ClipChildren | WindowStyle.ClipSiblings);

        this.ApplyExStyleBit(WsExDlgModalFrame, false);
        this.ApplyExStyleBit(WsExWindowEdge, false);
        this.ApplyExStyleBit(WsExClientEdge, false);
        this.ApplyExStyleBit(WsExStaticEdge, false);
        this.ApplyExStyleBit(WsExToolWindow, true);
        ...
        unsafe
        {
            ...
            int cornerPref = useRoundedCorners ? DwmwcpRound : DwmwcpDoNotRound;
            _ = DwmSetWindowAttribute(this._hwnd, DwmwaWindowCornerPreference, &cornerPref, sizeof(int));
        }
    ...
    protected void ApplyFullBleedHardening()
    {
        ...
        unsafe
        {
            ...
            _ = DwmSetWindowAttribute(this._hwnd, DwmwaNcRenderingPolicy, &ncrpDisabled, sizeof(int));
        }
        ...
        _ = DwmExtendFrameIntoClientArea(this._hwnd, ref margins);
    ...
    /// </summary>
    public void Show() => this.RaiseShow(null);
    ...
    public void Show(Transition transition) => this.RaiseShow(transition);
    ...
    private void RaiseShow(Transition? transition)
    {
        ...
        if (this.DispatcherQueue.HasThreadAccess)
        {
            ...
            this._seenActivated = false;
            this.EnsureCloakedBeforeShow();
        }
        ...
        this.DispatcherQueue.TryEnqueue(
            ...
            () =>
            {
                ...
                this.EnsureCloakedBeforeShow();
                this.EnsureInputHooks();

                // Cloaked mode is made SW_SHOWNA-visible only after cloaking succeeds.
                if (!this._cloakWhenHidden)
                {
                    _ = ShowWindow(this._hwnd, SwShowNa);
                }

                this.Showing?.Invoke(this, new ShowingEventArgs(transition));
            });
    }
    ...
    public void Hide()
    {
        this.DispatcherQueue.TryEnqueue(
            ...
            () =>
            {
                ...
                this.Hiding?.Invoke(this, args);
                args.RunWhenComplete(this.HideCore);
            });
    }
    ...
    /// <remarks>
    ///     <para>
    ///         A hidden WinUI 3 window renders nothing, so its composition surface keeps
    ...
    ///         keeps laying it out and painting it and there is no stale frame to put back.
    ///     </para>
    ///     <para>
    ///         This changes what the show sequence means: <see cref="Show()" /> still raises
    ...
    ///         and reveal a window that is correct in its first visible frame.
    ///     </para>
    ///     <para>
    ///         Enabling it also warms the window up: the XAML tree is built, templated and
    ///         painted right away rather than on the first summon.
    ///     </para>
    ///     <para>
    ///         Call this once, from the consumer's constructor after its content has been
    ///         set. Cloaking is a DWM feature; if DWM refuses, the window remains hidden and the
    ///         next <see cref="Show()" /> retries.
    ///     </para>
    /// </remarks>
    protected void EnableCloakedHide()
    {
        ...
        this.EnsureHiddenOwner();
        this._cloakWhenHidden = true;
        this.HideCore();
    }
    ...
    public void Reveal()
    {
        ...
        this.ApplyExStyleBit(WsExTransparent, false);
    }
    ...
    {
        if (this._cloakWhenHidden && !this._cloaked)
        {
            this.CloakAndKeepShown();
        }
    }
    ...
    {
        if (this._hiddenOwnerWindow is not null || this._hwnd == 0)
        {
            return;
        }

        this._hiddenOwnerWindow = new Window();
        nint hiddenOwnerHwnd = WindowNative.GetWindowHandle(this._hiddenOwnerWindow);
        _ = SetWindowLongPtr(this._hwnd, GwlpHwndParent, hiddenOwnerHwnd);

        // WS_EX_APPWINDOW overrides normal owner-based taskbar suppression.
        this.ApplyExStyleBit(WsExAppWindow, false);
    }
    ...
    private void CloakAndKeepShown()
    {
        ...
        _ = ShowWindow(this._hwnd, SwHide);
        if (!this.SetCloak(true))
        {
            return;
        }
        ...
        this.ApplyExStyleBit(WsExTransparent, true);
    ...
    private bool SetCloak(bool cloak)
    {
        ...
        {
            int value = cloak ? 1 : 0;
            return DwmSetWindowAttribute(this._hwnd, DwmwaCloak, &value, sizeof(int)) == 0;
        }
    }
    ...
    private void OnActivatedForDismiss(object sender, WindowActivatedEventArgs args)
    {
        ...
        {
            if (this.DismissOnFocusLost && this._seenActivated)
            {
                this.Hide();
            }
        ...
        this._seenActivated = true;
    }

    private void EnsureInputHooks()
    {
        if (this._inputHooked || this.Content is not UIElement element)
        {
            return;
        }

        element.KeyDown += this.OnContentKeyDown;
        this._inputHooked = true;
    }

    private void OnContentKeyDown(object sender, KeyRoutedEventArgs e)
    {
        if (this.DismissOnEscape && e.Key == VirtualKey.Escape)
        {
            e.Handled = true;
            this.Hide();
        }
    ...
    private void ApplyExStyleBit(int bit, bool set)
    {
        ...
        nint exStyle = GetWindowLongPtr(this._hwnd, GwlExStyle);
        nint updated = set ? exStyle | bit : exStyle & ~(nint)bit;
        if (updated != exStyle)
        {
            _ = SetWindowLongPtr(this._hwnd, GwlExStyle, updated);
        }
    }
    ...
    [LibraryImport("dwmapi.dll")]
    private static unsafe partial int DwmSetWindowAttribute(
        nint hwnd,
        int dwAttribute,
        void* pvAttribute,
        int cbAttribute);
...
public sealed class HidingEventArgs : EventArgs
{
    ...
    public Deferral GetDeferral()
    {
        ...
        return new Deferral(this.OnDeferralCompleted);
    }
    ...
    {
        if (this._raised && Volatile.Read(ref this._outstanding) == 0)
        {
